...
{
    ros::init(argc, argv, "pointgrey_left_rectify_js_node");
    ros::NodeHandle nh("~");
    nh.getParam("LeftCamID", CAM_serialNumber);
    ...
    //pub_image = it.advertise("/camera/left/image_raw", 1);
    

    if(getColorData)
        pub_imagebgr = it.advertise("/cam0/imagebgr_raw", 1);

    std::cout << "[Left]CAM_serialNumber: " << CAM_serialNumber << std::endl;
...
{    
    ...
    }else
    {
        ...
        cv::remap(image, image_rectify, RectifyMap_cam0_map1, RectifyMap_cam0_map2, cv::INTER_LINEAR);
        cv::imshow("[left] image", image);
        cv::imshow("[left] image_rectify", image_rectify);
        cv::waitKey(1);
